...
void CQuadraticProgram::InitializeProblemSize(const int& num_var, const int& num_cons)
{
    ...
    _H.setZero();
    _g.resize(_num_var,1);
    _g.setZero();
    _A.resize(_num_cons, _num_var);
    _A.setZero();
    _lbA.resize(_num_cons,1);
    _lbA.setZero();
    _ubA.resize(_num_cons,1);
    _ubA.setZero();
    _lb.resize(_num_var,1);
    _lb.setZero();
    _ub.resize(_num_var,1);
    _ub.setZero();
    _bInitialized = false;
    _num_state = 100;
    _Xopt.resize(_num_var,1);
    _Xopt.setZero();
    _comptime = 100000.0;
    _options.printLevel = PL_NONE;
    // cout << "num_var : "<<num_var << "  /  num_cons : "<<num_cons  <<endl;
}
