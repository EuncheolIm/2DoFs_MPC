...
{	
	...
	// D_d.segment(2,2) = -1 * dT * M.inverse() * (C + G - _torque_freq);
	...
	for (int i = 0; i <Np; i++){
		for (int j = 0; j <Np; j++){
			if( j<=i){
				Phi.block<4,2>(4*i,2*j) = temp_Phi_B.block<4,2>(2*_dof*(i-j),0);
			}
		}
	}
...
{	
	...
	cout << "Y_stack : "<<(Y_stack.segment(0,2)*RAD2DEG).transpose() <<endl;
	cout << "error : "<< (_q(0)*RAD2DEG - Y_stack(0)*RAD2DEG) << " "<< (_q(1)*RAD2DEG - Y_stack(1)*RAD2DEG)<<endl;
	...
	// _torque = x1;
	_torque = (_kp*(x1.segment(0,2) - _q) + _kd*(x1.segment(2,2) - _qdot)) ;//ßßß+ C + G;
...
{	
	...
	log.setZero();
	// _q_min << -40*DEG2RAD, -30*DEG2RAD, -1.8750, -1.8750;
	// _q_max <<  40*DEG2RAD,  30*DEG2RAD, 1.8750, 1.8750;
	_q_min << -40*DEG2RAD, -100*DEG2RAD, -1.8750, -1.8750;
	_q_max <<  20*DEG2RAD,  100*DEG2RAD, 1.8750, 1.8750;
